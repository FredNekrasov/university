...
int pw3AlKate() {
    for (int a = 1; a <= 10; a += 3) {
         ...
         * Они запоминают при каких значениях x 'y' является максимальной/минимальной
         */
        ...
        for (double x = 0.5; x < 2.1; x += 0.1) {
             ...
             * К тому же запоминаем в переменной maxX значение x, при котором y является максимальным значением
             * Также с minY и minX
             */
            if (y > maxY) {
                maxY = y;
